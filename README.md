...
brianlee 的個人 Life OS：管理信仰/召會生活、個人目標、每日行程、側專案開發的筆記與自動化工具。內容以中文為主，資料夾名稱也已全部改成中文（僅實際在跑的程式碼專案 `小專案/DocOutline/` 刻意保留英文）。
...
**我想知道今天/這週有什麼行程待辦**
在專案目錄下開 Claude Code，輸入 `/daily-sync`：掃描 `行程/` 裡雜亂的行程資料（截圖、課表、公告），整理成 `每日整理/<日期>/` 底下人讀的清單，同步到 Calendar.app / Reminders.app / Notes.app（經 iCloud 到 iPhone/iPad）。也可以只跑其中一步：`/daily-check`、`/ios-sync`、`/notes-sync`。詳細操作與常見狀況見 [使用手冊/每日行程同步-SOP.md](使用手冊/每日行程同步-SOP.md)。
...
**我想記錄與追蹤個人目標方向**
[人生大方向/興趣/interest.md](人生大方向/興趣/interest.md)（興趣）、[人生大方向/語言/language.md](人生大方向/語言/language.md)（語言學習）、[人生大方向/職涯/professional.md](人生大方向/職涯/professional.md)（職涯方向）。
...
- **信仰/召會生活** — `召會生活/`
- **個人目標管理** — `人生大方向/`
- **每日行程自動化** — `行程/` + `每日整理/` + `.claude/skills/`（daily-check、daily-sync、ios-sync、notes-sync）+ `使用手冊/`
- **側專案開發** — `小專案/`
- **工具與學習筆記** — `工具/`、`快捷鍵/`
