...
description: 手動的「一次跑完」入口——依序執行 sub-daily-check（整理 管理行程/ 待辦）、sub-apple-sync（同步到 Calendar/Reminders/Notes）。沒有排程，完全手動觸發。
---

依序完整跑一次每日整理流程（沒有自動排程，使用者手動輸入 `/core-daily-sync` 才會執行）：

1. 執行 `sub-daily-check` 的完整邏輯（見 `.claude/skills/sub-daily-check/SKILL.md`）：掃描 `管理行程/`，更新/確認 `每日重點整理/<today>/<today>.md` 與 `tasks.json`，保守搬移過期檔案，寫 log。
